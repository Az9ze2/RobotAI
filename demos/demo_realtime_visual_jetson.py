...
class VisualPipelineDemo:
    ...
    def draw_info_panel(self, frame, detections, tracks):
        ...
        has_triggered = any(hasattr(t, 'should_recognize') and t.should_recognize for t in tracks)
        has_head_pose = any(hasattr(t, 'head_pose') and t.head_pose for t in tracks)
        is_looking = any(hasattr(t, 'head_pose') and t.head_pose and t.head_pose.get('is_looking', False) for t in tracks)
    ...
    def process_frame(self, frame):
        ...
        for track in tracks:
            ...
            best_det = None
            for d in detections:
                iou = calc_iou(track.bbox, d["bbox"])
                if iou > best_iou:
                    best_iou = iou
                    best_det = d
            ...
            is_looking = track.head_pose.get('is_looking', False) if hasattr(track, 'head_pose') and track.head_pose else False
            ...
            track.should_recognize = decision.should_trigger
            track.trigger_reason = decision.reason  # Store reason for display
